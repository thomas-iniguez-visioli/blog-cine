...
const runAction = (name) => {
	...
	if(os.platform()==="win32"){
		...
		//	writeFileSync(`..\\cdn\\${name}.jpg`,result.url)
			run(`dir ./cdn && curl "${result.replace('&amp;s','')}" -o ./cdn/${name}.jpg >${name}.log && git clocd ./cdn &&git add * * &&git commit -m"${name}" --amend &&git pull origin main --rebase  &&git push origin main`,__dirname+"")
		}).catch((err)=>{log(err)})
