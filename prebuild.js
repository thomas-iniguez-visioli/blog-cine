...
const runAction = (name) => {
	...
	if(os.platform()==="win32"){
		...
		//	writeFileSync(`..\\cdn\\${name}.jpg`,result.url)
			run(`dir ./cdn && curl "${result.replace('&amp;s','')}" -o ./cdn/${name+new DA}.jpg >${name}.log &&cd ./cdn &&git add * *  &&git commit -m"${name}" --amend &&git pull origin main --rebase  &&git push origin main`,__dirname+"")
		}).catch((err)=>{log(err)})
