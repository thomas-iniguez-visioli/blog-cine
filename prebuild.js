...
const runAction = (name) => {
	...
	if(os.platform()==="win32"){
		...
		//	writeFileSync(`..\\cdn\\${name}.jpg`,result.url)
			run(`dir ./cdn && curl "${result.replace('&amp;s','')}" -o ./cdn/${name+new Date().toDateString}.jpg >${name}.log &&cd ./cdn &&git add * *  &&git commit -m"${name}" --amend &&git pull origin main --rebase  &&git push origin main`,__dirname+"")
		}).catch((err)=>{log(err)})
		
		//run(`curl https://thomas-iniguez-visioli.github.io/cdn/${name}.jpg -o dist/${name+new Date().toISOString()}.jpg `,__dirname)
	}
	parsefile("./add.txt")
